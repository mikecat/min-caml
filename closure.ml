...
let rec g env known = function (* クロージャ変換ルーチン本体 (caml2html: closure_g) *)
  ...
  | KNormal.Var(x) -> Var(x)
  | KNormal.LetRec((mx, mt), defs, e2) -> (* 関数定義の場合 (caml2html: closure_letrec) *)
      (* 関数定義let rec x y1 ... yn = e1 in e2の場合は、
         xに自由変数がない(closureを介さずdirectに呼び出せる)
         と仮定し、knownに追加して最初の定義のe1をクロージャ変換してみる *)
      let { KNormal.name = (x, t); KNormal.args = yts; KNormal.body = e1 } = List.hd defs in
      ...
      (* envとenv'の差はxを入れたかどうかだけで、xは自由変数ではないので関係ない *)
      let zts = List.map (fun z -> (z, M.find z env)) zs in (* ここで自由変数zの型を引くために引数envが必要 *)
      (* ここでxをknownに追加するべきかがわかったので、その情報を利用して各定義の変換を行う *)
      ...
      if S.mem mx (fv e2') then (* mxが変数としてe2'に出現するか *)
        MakeCls((mx, mt), { entry = Id.L(mx); actual_fv = zs }, e2') (* 出現していたら削除しない *)
      else
